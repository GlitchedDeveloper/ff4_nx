/* movie_player.c -- FFmpeg + GLES1 movie overlay for FF4 3D */
...
static int open_codec(AVCodecContext** out, AVStream* stream, int threads) {
    ...
    if (!ctx)
        return -1;
    if (avcodec_parameters_to_context(ctx, stream->codecpar) < 0 || (ctx->thread_count = threads, avcodec_open2(ctx, codec, NULL)) < 0) {
...
static void push_audio_frame(const AVFrame* frm) {
    ...
    mtx_lock(&mp.lock);
    if (!mp.audio_primed && opensles_movie_samples_queued() * 1000 >= (uint64_t)mp.audio_rate * AUDIO_PREFILL_MS) {
        mp.audio_primed = 1;
...
static int setup_audio(void) {
    ...
    if (swr_alloc_set_opts2(&mp.swr, &out_layout, AV_SAMPLE_FMT_S16, mp.audio_rate,
            &mp.actx->ch_layout, mp.actx->sample_fmt, mp.actx->sample_rate, 0, NULL)
        < 0)
        mp.swr = NULL;
#else
    const uint64_t in_layout = mp.actx->channel_layout ? mp.actx->channel_layout : av_get_default_channel_layout(mp.actx->channels);
...
void movie_render(void) {
    ...
    float x0 = 0.0f, y0 = 0.0f, x1 = SCREEN_WIDTH, y1 = SCREEN_HEIGHT;
    const float va = (float)mp.width / (float)mp.height;
    const float sa = SCREEN_WIDTH / SCREEN_HEIGHT;
    if (va > sa) {
        const float h = SCREEN_WIDTH / va;
        y0            = (SCREEN_HEIGHT - h) * 0.5f;
        y1            = y0 + h;
    } else {
        const float w = SCREEN_HEIGHT * va;
        x0            = (SCREEN_WIDTH - w) * 0.5f;
        x1            = x0 + w;
    }
